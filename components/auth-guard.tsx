...
import { getCurrentTestUser } from "@/lib/test-auth";

function subscribeAuthStore(onStoreChange: () => void) {
  window.addEventListener("storage", onStoreChange);
  window.addEventListener("nextpost-auth-change", onStoreChange);
  return () => {
    window.removeEventListener("storage", onStoreChange);
    window.removeEventListener("nextpost-auth-change", onStoreChange);
  };
}

function getAuthSnapshot() {
  return getCurrentTestUser()?.id ?? "";
}

export function AuthGuard({ children }: { children: React.ReactNode }) {
  const router = useRouter();
  const pathname = usePathname();
  const userId = useSyncExternalStore(subscribeAuthStore, getAuthSnapshot, () => "");
